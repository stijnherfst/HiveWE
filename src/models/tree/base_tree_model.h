...
class BaseTreeModel : public QAbstractProxyModel {
	...
	void rowsInserted([[maybe_unused]] const QModelIndex& parent, int first, [[maybe_unused]] int last) {
		assert(first == last);
	...
	void rowsRemoved([[maybe_unused]] const QModelIndex& parent, int first, [[maybe_unused]] int last) {
		assert(first == last);
	...
	virtual BaseTreeItem* getFolderParent(const std::string&) const {
		return nullptr;
	};
	...
	virtual DropChange prepareDrop(const std::string&, const BaseTreeItem*) const {
		return {};
	};
	...
	void applyCopy(const DropChanges& changes) {
		...
		for (const auto& [id, fields] : changes) {
			...
			copy_table_model_row(sourceModel(), id, new_id);
  ...
  public:
	...
	bool canDropMimeData(const QMimeData* data, const Qt::DropAction action, const int, const int, const QModelIndex& parent) const override {
		QString question;
