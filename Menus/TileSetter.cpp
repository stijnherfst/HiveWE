...
TileSetter::TileSetter(QWidget *parent) : QDialog(parent) {
	...
	for (auto&& i : map->terrain.tileset_ids) {
		const auto image = resource_manager.load<Texture>(slk.data("dir", i) + "\\" + slk.data("file", i) + (hierarchy.hd ? "_diffuse.dds" : ".dds"));
...
void TileSetter::update_available_tiles() const {
	...
	for (auto&&[key, value] : map->terrain.terrain_slk.header_to_row) {
		...
		const auto image = resource_manager.load<Texture>(slk.data("dir", key) + "\\" + slk.data("file", key) + (hierarchy.hd ? "_diffuse.dds" : ".dds"));
		const auto icon = ground_texture_to_icon(image->data.data(), image->width, image->height);
